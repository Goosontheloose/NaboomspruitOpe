...
with tab3:
    ...
    with st.form("score_entry_form"):
        ...
        for p in players:
            ...
            st.markdown(f"**{p}** <span class='stroke-badge'>Strokes: {get_allowed_strokes(p, hole_to_edit)}</span>", unsafe_allow_html=True)
            c1, c2, c3, c4, c5, c6 = st.columns(6)
            temp_df.at[idx, 'Score'] = c1.number_input("Score", 0, 15, value=int(df.at[idx, 'Score']), key=f"s{p}{hole_to_edit}")
            temp_df.at[idx, 'Drinks'] = c2.number_input("Drinks", 0, 10, value=int(df.at[idx, 'Drinks']), key=f"d{p}{hole_to_edit}")
            temp_df.at[idx, 'Camo'] = c3.checkbox("Camo", value=(str(df.at[idx, 'Camo']).lower() == "true"), key=f"c{p}{hole_to_edit}")
            temp_df.at[idx, 'Throw'] = c4.checkbox("Throw", value=(str(df.at[idx, 'Throw']).lower() == "true"), key=f"t{p}{hole_to_edit}")
            temp_df.at[idx, 'Kick'] = c5.checkbox("Kick", value=(str(df.at[idx, 'Kick']).lower() == "true"), key=f"k{p}{hole_to_edit}")
            temp_df.at[idx, 'Mully'] = c6.number_input("Mully", 0, 2, value=int(df.at[idx, 'Mully']), key=f"m{p}{hole_to_edit}")
            st.divider()
